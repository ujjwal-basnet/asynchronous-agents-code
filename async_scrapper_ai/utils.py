...
# ---------- pricing ----------
## imaginary numbers because , when i divide / 1 milllion the nunber is so small that i can't count so increasing prices
...
def calculate_usage_report(message: Message) -> MessageReport:
    ...
    req_tokens = count_tokens(message.prompt, message.model)### website price is per million token so
    res_tokens = count_tokens(message.response, message.model)
    req_cost = price * req_tokens / 1_000_000
    res_cost = price * res_tokens / 1_000_000 ## price per 1 million tokens so
